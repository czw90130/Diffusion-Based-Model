...
def depth2normal_perse(depth, intrinsics):  # 深度图，内参
    global flag_XY, X, Y
    # depth: [B,1,H,W]
...
# reproject the 3D points to orthographic view 重投影到正交视角
# _ _ 透视内参 正交参数
...
def perse2ortho(depth_p, color_p, intrinsics_p, intrinsics_o):
    ...
    x_cord_p = (X1 - cx) * inv_fx * depth  # 坐标
    y_cord_p = (cy - Y1) * inv_fy * depth

    p = torch.stack([x_cord_p, y_cord_p, depth, color_p[:, 0, :, :],
                     color_p[:, 1, :, :], color_p[:, 2, :, :]], dim=3)  # (B,H,W,3)
...
def dilate(depth, pix):  # 扩张
    # depth: [B, 1, H,W]
    newdepth = depth.clone().detach()
    for i in range(pix):
        d1 = newdepth[:, :, 1:, :]
        d2 = newdepth[:, :, :-1, :]
        d3 = newdepth[:, :, :, 1:]
        d4 = newdepth[:, :, :, :-1]
        newdepth[:, :, :-1, :] = torch.where(
            newdepth[:, 0:1, :-1, :] > 0, newdepth[:, :, :-1, :], d1)
        newdepth[:, :, 1:, :] = torch.where(
            newdepth[:, 0:1, 1:, :] > 0, newdepth[:, :, 1:, :], d2)
        newdepth[:, :, :, :-1] = torch.where(
            newdepth[:, 0:1, :, :-1] > 0, newdepth[:, :, :, :-1], d3)
        newdepth[:, :, :, 1:] = torch.where(
            newdepth[:, 0:1, :, 1:] > 0, newdepth[:, :, :, 1:], d4)
        depth = newdepth
    return newdepth


def erode(depth, pix):  # 侵蚀
    # depth: [B, C, H, W]
    newdepth = depth.clone().detach()
    for i in range(pix):
        d1 = depth[:, :, 1:, :]
        ...
        d4 = depth[:, :, :, :-1]
        newdepth[:, :, :-1,
                 :] = torch.where(newdepth[:, :, :-1, :] > 0, d1, newdepth[:, :, :-1, :])
        newdepth[:, :, 1:, :] = torch.where(
            newdepth[:, :, 1:, :] > 0, d2, newdepth[:, :, 1:, :])
        newdepth[:, :, :, :-
                 1] = torch.where(newdepth[:, :, :, :-1] > 0, d3, newdepth[:, :, :, :-1])
        newdepth[:, :, :, 1:] = torch.where(
            newdepth[:, :, :, 1:] > 0, d4, newdepth[:, :, :, 1:])
        depth = newdepth
    return newdepth
...
def getEdgeFaces(mask, fp_idx, bp_idx):
    ...
    for i in range(len(contours)):
        ...
        bp_nextedge_idx = bp_idx[nextedges[:, 1],
                                 nextedges[:, 0]].reshape(-1, 1)
        fp_nextedge_idx = fp_idx[nextedges[:, 1],
                                 nextedges[:, 0]].reshape(-1, 1)
        ...
        if i == 0:
            ...
            all_boundary_faces_idx = np.vstack(
                (all_boundary_faces_idx, boundary_faces_idx))
...
def getbackFaces(mask, p_idx):
    ...
    fp_faces = all_faces[np.where(
        all_faces[:, 0] * all_faces[:, 1] * all_faces[:, 2] > 0)]
    return fp_faces
...
def fuvidbatch2color(ubatch, vbatch, wbatch, uvmap, batch_size, H, pred_flg=False):
    # u or v: [B,1,H,H]
    # uvmap or color: [B,3,H,H]
    # print("ubatch: ", ubatch.shape)
    ubatch = (ubatch + 1) / 2
    ubatch[ubatch<0] = 0
    ubatch[ubatch>=1] = 1
    vbatch = (vbatch + 1) / 2
    ...
    wbatch[wbatch>=1] = 1
    wbatch *= 255
    colorbatch = torch.zeros(
        batch_size, H, H, 3)

    y_cord, x_cord = torch.meshgrid(torch.tensor(range(H)).repeat(
        batch_size), torch.tensor(range(H)))
    x_cord = x_cord.long()
    y_cord = y_cord.long()

    u = (ubatch*uvmap.shape[2]*0.999).long()
    v = (vbatch*uvmap.shape[1]*0.999).long()
    pred_gap = 0
    if pred_flg:
        pred_gap = 7
    w = ((wbatch+pred_gap)//14).long()

    b,_ = torch.meshgrid(torch.tensor(range(batch_size)), torch.tensor(range(H*H)))
    b = b.flatten().long()

    colorbatch[b, y_cord.flatten(), x_cord.flatten(),
               :] = uvmap[w.flatten(), v.flatten(), u.flatten(), :]

    return colorbatch.permute(0, 3, 1, 2).cuda()

def uvbatch2color(ubatch, vbatch, uvmap, batch_size, H):
    # u or v: [B,1,H,H]
    # uvmap or color: [B,3,H,H]
    # print("ubatch: ", ubatch.shape)
    ubatch = (ubatch + 1) / 2
    ...
    u = (ubatch*uvmap.shape[1]*0.999).long()
    v = (vbatch*uvmap.shape[0]*0.999).long()

    b,_ = torch.meshgrid(torch.tensor(range(batch_size)), torch.tensor(range(H*H)))
    b = b.flatten().long()

    colorbatch[b, y_cord.flatten(), x_cord.flatten(),
               :] = uvmap[v.flatten(), u.flatten(), :]

    return colorbatch.permute(0, 3, 1, 2).cuda()

def test_uv2color(u_path_list, v_path_list, uvmap_path, out_path):
    ubatch = torch.zeros(len(u_path_list), 1, 424, 424)
    vbatch = torch.zeros(len(u_path_list), 1, 424, 424)
    for i,p in enumerate(u_path_list):
        uimg = cv2.imread(p, cv2.IMREAD_UNCHANGED)
        uimg = uimg.astype(np.float32)
        uimg /= 10000
        ubatch[i] = torch.tensor(uimg)
    for i,p in enumerate(v_path_list):
        vimg = cv2.imread(p, cv2.IMREAD_UNCHANGED)
        vimg = vimg.astype(np.float32)
        vimg /= 10000
        vbatch[i] = torch.tensor(vimg)
    uvmap = torch.tensor(cv2.imread(uvmap_path).astype(np.float32))

    rt = uvbatch2color(ubatch, vbatch, uvmap)

    for i in range(rt.shape[0]):
        cimg = rt[i]
        cimg = cimg.permute(1,2,0).int()
        cv2.imwrite(os.path.join(out_path,"cimg_"+str(i)+".png"), cimg.numpy())
